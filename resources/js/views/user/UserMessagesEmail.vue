<template>
    ...
    <div class="container-fluid" style="overflow-x: auto;">
        ...
        >
            ...
            <v-th sortKey="user_phone"style="width: 150px">Phone</v-th>
            </thead>
            ...
            <tr>
                ...
                <td><input class="form-input input-lg" style="width: 150px" v-model="filters.user_phone.value"  placeholder="Select by user_phone"></td>
            ...
            <tr v-for="row in displayData" :key="row.id"  style="width: 30px" >
                ...
                <td><input id="user_phone" class="form-control" style="width: 150px" v-model.text="row.user_phone" readonly></td>
...
export default {
    ...
    methods: {
       getMessages() {
        ...
        destroyMessage(id) {
            axios.delete(`/emails/destroy/${id}`)
                .then(response => {
...
// Обновление сообщения по id
        updateMessage(id, text_message, sent_message_info) {
            ...
            axios.get(`/emails/update/${id}/${encodedNewMessage}/${sent_message_info}`)
                .then(response => {
                    // Проверяем успешность операции
        ...
        newMessageButton() {
            this.$router.push('/admin/new_message_email');
        },
        ...
        sendMessage() {
            ...
            const url = `/repeatEmail/${this.selectedEmails.join(',')}`
            axios.get(`/repeatEmail/${this.selectedEmails.join(',')}`)
                .then(response => {
                    // Проверяем успешность операции
                    if (response.status === 200) {
