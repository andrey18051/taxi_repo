                ...
                <tr v-for="row in displayData" :key="row.id">
                    ...
                    <td>
                        <div class="btn-group" role="group">
                            <button class="btn btn-success" @click="editCities(row.id, row.name, row.address, row.login, row.password, row.online, row.card_max_pay, row.bonus_max_pay, row.payment_flow)" style="margin-left: 5px">
...
export default {
    ...
    methods: {
        ...
        editCities(id, name, address, login, password, online, card_max_pay, bonus_max_pay, payment_flow) {
            axios.get('/city/edit/'+ id +'/'+name+'/'+address+'/'+login+'/'+password+'/'+ online+'/'+card_max_pay+'/'+ bonus_max_pay+'/'+ payment_flow)
                .then(function(ret) {
