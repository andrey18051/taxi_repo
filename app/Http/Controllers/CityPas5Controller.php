...
class CityPas5Controller extends Controller
{
...
//    }
    ...
    {
        return response()->json(City_PAS5::get());
    }
    ...
    {
        return City_PAS5::where('name', $city)->get()->toArray();
    }
    ...
    public function cityOnline($city)
    {
        ...
        foreach ($serverArr as $value) {
            ...
            $city = City_PAS5::where('address', $value["address"])->first();
...
//                }
    ...
    {
//Разморозка
        $serverFalse = City_PAS5::where('name', $city)
        ...
        if (!$serverFalse->isEmpty()) {
            ...
            foreach ($serverArr as $value) {
                ...
                if ($timeFive) {
                    $cityRecord = City_PAS5::find($value["id"]);
        ...
        $server = City_PAS5::where('name', $city)
...
//        $response = curl_exec($curlInit);
        ...
        $response = curl_exec($curlInit);
//dd($domainFull);
        $city = City_PAS5::where('address', $domain)->first();
...
//                    $alarmMessage->sendMeMessage($messageAdmin);
    ...
    {
        $cities = City_PAS5::all()->toArray();
        foreach ($cities as $value) {
            $city = City_PAS5::where('name', $value["name"])->first();
