...
class CityPas1Controller extends Controller
{
...
//    }
    ...
    {
        return response()->json(City_PAS1::get());
    }
    ...
    {
        return City_PAS1::where('name', $city)->get()->toArray();
    }
...
//dd($serverArr);
        foreach ($serverArr as $value) {
            ...
            $city = City_PAS1::where('address', $value["address"])->first();
//            dd($value["address"]);
//            dd( $timeFive);
            if ($online == "false") {
                if ($timeFive == true) {
...
//                        $city->save();
    ...
    {
//Разморозка
        $serverFalse = City_PAS1::where('name', $city)
        ...
        if (!$serverFalse->isEmpty()) {
            ...
            foreach ($serverArr as $value) {
                ...
                if ($timeFive) {
                    $cityRecord = City_PAS1::find($value["id"]);
        ...
        $server = City_PAS1::where('name', $city)
...
//        $response = curl_exec($curlInit);
        ...
        $response = curl_exec($curlInit);
//dd($domainFull);
        $city = City_PAS1::where('address', $domain)->first();
...
//                    $alarmMessage->sendMeMessage($messageAdmin);
    ...
    {
        $cities = City_PAS1::all()->toArray();
        foreach ($cities as $value) {
            $city = City_PAS1::where('name', $value["name"])->first();
