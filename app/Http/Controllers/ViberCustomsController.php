...
class ViberCustomsController extends Controller
{

    public function viberCustoms() {
        return redirect('viber://pa?chatURI=mycustomsbot');
    }

    public function setWebhook(ViberCustoms $viber)
    {
        $ch = $viber->setWebhook();
        dd(json_decode($ch->body()));
    }

    public function getAccountInfo(ViberCustoms $viber)
    {
        $ch = $viber->getAccountInfo();
        dd(json_decode($ch->body()));
    }

    public function getUserDetails(ViberCustoms $viber, $user_id)
    {
    ...
    public function sendMessage(ViberCustoms $viber, $user_id, $message)
    {
        $viber->sendMessage($user_id, $message);
    }

    public function sendKeyboard(ViberCustoms $viber, $user_id, $message, $keyboard)
    {
