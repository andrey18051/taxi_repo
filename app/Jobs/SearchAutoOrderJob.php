...
class SearchAutoOrderJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            // Валидация UID
            if (empty($this->uid)) {
                Log::error('SearchAutoOrderJob: Invalid UID provided');
                return;
            }

            // Вызов сервиса для обработки
            $result = (new \App\Http\Controllers\UniversalAndroidFunctionController)->searchAutoOrderJob($this->uid, "yes_mes");

            // Логирование результата
            Log::info('SearchAutoOrderJob completed', [
                'uid' => $this->uid,
                'status' => $result['status'],
                'message' => $result['message'],
            ]);

            $messageAdmin = "SearchAutoOrderJob completed: " . $result['message'];
            (new MessageSentController)->sentMessageAdmin($messageAdmin);

        } catch (\Exception $e) {
            Log::error('SearchAutoOrderJob failed', [
                'uid' => $this->uid,
                'error' => $e->getMessage(),
            ]);
